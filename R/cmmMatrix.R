...
#' @export
rbind.cmmMatrix<-function(...,vmode='byte',folderOut=NULL,nChunks=NULL,dimorder=c(2,1),deparse.level=1){
    allargs<-list(...)
    ...
    mmmatrix<-cmmMatrix(nrow=numrows,ncol=numcols,vmode=vmode,folderOut=folderOut,nChunks=nChunks,dimorder=dimorder)
    currow<-1
...
#' @export
cbind.cmmMatrix<-function(...,vmode='byte',folderOut=NULL,nChunks=NULL,dimorder=c(2,1),deparse.level=1){
    allargs<-list(...)
    alldims<-unique(sapply(allargs,function(arg){
        dims<-dim(arg)
        if(is.null(dims)){
